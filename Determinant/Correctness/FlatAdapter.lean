...
theorem get_eq_ofFlatArray
    ...
    (hA : A.size = n * n)
    (i j : Fin n) :
    get n A i.val j.val = ofFlatArray (n := n) (m := n) A hA i j := by
...
theorem iter_eq_correctness
    ...
    iter n A t F i.val j.val =
      Correctness.iterEntry (ofFlatArray (n := n) (m := n) A hA) t F' i j := by
  ...
  | succ t ih =>
      ...
      have hdiag :
          (∑ k : Fin n, if i.val < k.val then iter n A t F k.val k.val else 0) =
            ∑ k : Fin n,
              if i.val < k.val then
                Correctness.iterEntry (ofFlatArray (n := n) (m := n) A hA) t F' k k
              else 0 := by
      ...
      have hoff :
          (∑ k : Fin n,
            ...
            ∑ k : Fin n,
              if i.val < k.val then
                Correctness.iterEntry (ofFlatArray (n := n) (m := n) A hA) t F' i k *
                  ofFlatArray (n := n) (m := n) A hA k j
              else 0 := by
        apply Finset.sum_congr rfl
        intro k _hk
...
theorem birdDet_eq_birdDetSpec_ofFlatArray
    ...
    (A : Array R)
    (hA : A.size = n * n) :
    birdDet n A = Correctness.birdDetSpec (ofFlatArray (n := n) (m := n) A hA) := by
  cases n with
  | zero =>
      rfl
  | succ k =>
      ...
      exact congrArg ((-1 : R) ^ k * ·)
        (iter_eq_correctness (k + 1) A hA k (get (k + 1) A)
          (fun i j => ofFlatArray (n := k + 1) (m := k + 1) A hA i j)
